...
static spi_buffer spi2_ms_tx_buffer = {
    .dt_count = RXTX_BUFFER_SIZE,
};
...
static int spi2_ms_rx_handler(struct lthread *self)
{
    /* обработка полученных данных */
    return 0;
}
static int spi2_ms_tx_handler(struct lthread *self)
{
    /* действия после отправки данных */
    return 0;
}
EMBOX_UNIT_INIT(spi2_ms_init);
static int spi2_ms_init(void)
{
  ...
  */
    ...
    res = irq_attach(15, spi2_ms_dma_tx_irq_handler, 0, NULL, "tim_irq_handler");
    res = irq_attach(14, spi2_ms_dma_rx_irq_handler, 0, NULL, "tim_irq_handler");

    lthread_init(&spi2_ms_tx_buffer.dt_lth, &spi2_ms_tx_handler);
    lthread_init(&spi2_ms_rx_buffer.dt_lth, &spi2_ms_rx_handler);
...
static irq_return_t spi2_ms_dma_tx_irq_handler(unsigned int irq_nr, void *data)
{
    if (LL_DMA_IsActiveFlag_TC5(DMA1) != RESET)
    {
        ...
        //
        lthread_launch(&spi2_ms_tx_buffer.dt_lth);
    }
    return IRQ_HANDLED;
}
STATIC_IRQ_ATTACH(15, spi2_ms_dma_tx_irq_handler, NULL);
static irq_return_t spi2_ms_dma_rx_irq_handler(unsigned int irq_nr, void *data)
{
    if (LL_DMA_IsActiveFlag_TC4(DMA1) != RESET)
    {
        ...
        lthread_launch(&spi2_ms_rx_buffer.dt_lth);
    }
    return IRQ_HANDLED;
}
STATIC_IRQ_ATTACH(14, spi2_ms_dma_rx_irq_handler, NULL);
